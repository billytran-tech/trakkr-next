...
When all these steps are completed, make sure you have run `npm install` both inside your project directory as your `/functions` folder and run the development server:

```bash
npm run install:initial
```
...
Make sure you add this code inside `config/firebase.ts` to let the application use the functions emulator.
...
Learn more: https://firebase.google.com/docs/functions/local-emulator#web-version-9
